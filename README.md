...
| 档位 | 画面与帧率上限 | 适合用途 |
| --- | --- | --- |
| 高清 | 原始录制尺寸，最高 60 帧 | 优先保留画面细节。 |
| 日常（默认） | 1080p，30 帧 | 日常演示与分享。 |
| 小巧 | 720p，30 帧 | 进一步节省空间。 |
| 极小 | 480p，24 帧 | 随手记录，小字细节会减少。 |
| 自定义 | 根据时长和 MB 上限适配 | 文件有明确大小限制。 |

保留画面比例，不放大低分辨率素材。界面会显示预计大小；复杂画面在小体积档可能进一步降低尺寸。自定义 MB 是**每个 MP4 的上限**，独立音频文件另计。

高清会尝试保守压缩；如果文件反而增大，或抽样画面损失过多，则保留原片。不承诺所有录屏都能在清晰度不变的情况下大幅缩小。编码依据与实测见[导出流程与体积优化](docs/export-redesign.md)。
